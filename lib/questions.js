...
const db = require("../config/database"); // Database attachment
...
const userActions = {
  ...
  userAddEmployee() {
    ...
    inquirer.prompt(questionsAddEmployee).then((answers) => {
      // let addedEmployee = answers.newEmployee;
      // let employeeAdded = [];
      // resp.forEach(newEmployee) => {
      //   employeeAdded.push(newEmployee.questionsAddEmployee); //! ??
      // }
      // let
      // loop through the values in the database table...but pull out the values in the columns and stick it into name, id into value...for every item in the database
      //
      // const roleChoices = [];
      // const roles =
      // return answers;
...
// Loop the values from the database and put it in "name" and "value" .forEach or .map
