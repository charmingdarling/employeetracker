...
//Create a table
// Takes an array of objects and displays a table. :)
// Each item in the array will be a row
// The properties / keys become headings on the table
// console.table(array name);

// 'db' object is being accepted as an argument
...
//Query Functions
// TODO: I suspect that I need to add template literals or something to get the actual query to show the right table.

const sql = "SELECT * FROM ?"; // Global variable

function viewDepartments(db) {
  // Accepting 'db' object as an argument
  // const query = `SELECT * FROM neonatal_departments`;
  db.query(sql, neonatal_departments, (err, res) => {
    // Passing the 'db' object as an argument
    if (err) {
      console.log(err);
    }
    console.table(sql);
...
function viewEmployees(db) {
  // Accepting 'db' object as an argument
  db.query(sql, employees, (err, res) => {
    if (err) {
      console.log(err);
    }
