...
FROM
  ...
  LEFT JOIN neonatal_departments d ON r.department_id = d.id;`;
  db.query(sql, (err, res) => {
    if (err) {
...
async function userAddEmployee() {
  ...
  const managerNames = await userGenerateEmployeeChoices();
  const questionsAddEmployee = [
    ...
    },
    {
      ...
      type: "list",
      message: "What is their role?",
      choices: await userGenerateRoleChoices(),
    },
  ];

  inquirer.prompt(questionsAddEmployee).then((answers) => {
    // Extract answers
    const { firstName, lastName, manager_id, role_id } = answers;

    // Construct an SQL INSERT statement
    const sql =
      "INSERT INTO employees (firstName, lastName, manager_id, role_id) VALUES (?, ?, ?, ?)";
    const values = [firstName, lastName, manager_id, role_id]; // Replace with actual column names and values.
...
async function userGenerateEmployeeChoices() {
  const managerNames = [];
  const sql = `SELECT id, CONCAT(e.firstName, ' ', e.lastName) AS Manager_Name FROM employees e WHERE manager_id IS NULL;`;
...
async function userGenerateRoleChoices() {
  const roles = [];
  ...
  const queryPromise = new Promise((resolve, reject) => {
    db.query(sql, (err, resp) => {
      if (err) {
        ...
        resolve(roles); // Resolve with the roles array
      }
    });
  });

  // Wait for the database query to complete before resolving with roles
  return await queryPromise;
}

async function updateEmployee() {
  const { id, manager_id, role_id } = await inquirer.prompt([
    {
      name: "id",
      type: "list",
      message: "Which employee would you like to update?",
      choices: await userGenerateEmployeeChoices(),
    },
    {
      name: "role_id",
      type: "list",
  ...
  db.query(sql, [role_id, manager_id, id], (err, resp) => {
    if (err) {
      console.error("Error executing the query:", err);
    } else {
